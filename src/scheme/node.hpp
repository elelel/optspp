  ...
  namespace scheme {
    namespace tree {
      ...
      void node::validate() const {
        for (const auto& p : parents_) {
          ...
          case KIND::DESCRIPTOR:
            dynamic_cast<const descriptor*>(n)->validate();
            break;
          case KIND::NAME:
            dynamic_cast<const name*>(n)->validate();
            break;
          case KIND::VALUE:
            dynamic_cast<const value*>(n)->validate();
            break;
          }
        }
      }
      ...
      node& node::add_child_name(const std::shared_ptr<name>& n) {
        std::cout << "Adding child '" << n->long_name() << "'\n";
